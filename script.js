...
// --- CRITICAL: Clear stale sessions on every page load ---
(async () => {
...
authForm.addEventListener('submit', async (e) => {
  ...
  if (isLoginMode) {
    ...
    else { currentUser = data.user; setDisplayName(); await loginSuccess(); }
  } else {
...
logoutBtn.addEventListener('click', async () => {
  ...
  } catch { console.log('Forced logout'); }
  finally {
    currentUser  = null;
    filesData    = [];
    selectedFile = null;
...
// --- 7. Auto-login if valid session exists ---
...
async function loadFiles() {
  ...
  if (error) { console.error('Load files error:', error.message); return; }
  filesData = data || [];
}
...
async function handleUpload(file) {
  ...
  const { error: storageError } = await db.storage.from('uploads').upload(filePath, file);
  if (storageError) {
  ...
  const { error: dbError } = await db.from('files').insert({
    ...
    type: type, url: urlData.publicUrl, user_id: currentUser.id
  });
...
async function deleteFile(id) {
  ...
  if (!error) { await loadFiles(); updateAllViews(); }
}
...
function renderFilesTable() {
  ...
  if (filesData.length === 0) {
    fileTableBody.innerHTML = `
      <tr><td colspan="4" style="text-align:center;padding:50px 0;color:#9ca3af;">
  ...
  filesData.forEach(file => {
    ...
    row.innerHTML = `
      <td><div class="file-name-cell"><i class="${getFileIconClass(file.type)} file-icon"></i><span>${file.name}</span></div></td>
      <td>${file.size}</td>
      <td>${date}</td>
...
navLinks.forEach(link => {
  link.addEventListener('click', function (e) { e.preventDefault(); switchView(this.dataset.view); });
});
